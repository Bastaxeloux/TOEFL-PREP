class Task4Manager {
    ...
    async loadPromptsList() {
        try {
            ...
            this.prompts.forEach(prompt => {
                ...
                // Create a short preview using notes or audio filename
    ...
    handlePromptSelect(event) {
        ...
        if (promptId) {
            ...
            if (prompt) {
                ...
                // Display notes in readonly div
                const notesDisplay = document.getElementById('notesDisplay');
                if (prompt.notes) {
                    notesDisplay.textContent = prompt.notes;
                    notesDisplay.style.color = '#333';
                } else {
                    notesDisplay.innerHTML = '<em style="color: #999;">No notes for this prompt</em>';
                }
            ...
            this.notes = '';
            const notesDisplay = document.getElementById('notesDisplay');
            notesDisplay.innerHTML = '<em style="color: #999;">Random mode - content will be selected when you start the task</em>';
    ...
    openModalForNew() {
        ...
        document.getElementById('modalNotesText').value = '';
    ...
    openModalForEdit() {
        ...
        document.getElementById('modalQuestionText').value = prompt.question || '';
        document.getElementById('modalNotesText').value = prompt.notes || '';
    ...
    async savePrompt() {
        ...
        try {
            ...
            const notesText = document.getElementById('modalNotesText').value;

            // Notes are optional, but we need audio
            if (!this.audioFile) {
    ...
    startTask() {
        ...
        if (!this.currentPromptId && this.prompts.length > 0) {
            ...
            this.question = randomPrompt.question || '';
            this.notes = randomPrompt.notes || '';
            this.audioFile = randomPrompt.audio_file;
            this.hasAudio = !!randomPrompt.audio_file;
        }

        // For Task 4, only audio is required
        if (!this.hasAudio) {
            alert('Please add at least one prompt with audio before starting the task.');
            return;
        }
        ...
        // Task 4 has no reading phase, go directly to listening
        this.startListening();
    }
    ...
    startPreparation() {
        ...
        // Display question
        const questionDiv = document.getElementById('preparationQuestion');
        if (this.question && this.question.trim()) {
            questionDiv.textContent = this.question;
        } else {
            questionDiv.textContent = 'Using points and examples from the lecture, summarize what you heard.';
        }
    ...
    async startRecording() {
        ...
        // Display question
        const questionDiv = document.getElementById('recordingQuestion');
        if (this.question && this.question.trim()) {
            questionDiv.textContent = this.question;
        } else {
            questionDiv.textContent = 'Using points and examples from the lecture, summarize what you heard.';
        ...
        try {
            const response = await fetch('/api/vocabulary_cards', {
                ...
                body: JSON.stringify({
                    ...
                    question: `Task 4 - ${this.notes ? this.notes.substring(0, 100) : 'Lecture'}...`,
                    title: cardTitle,
                    content: content
                })
            });
